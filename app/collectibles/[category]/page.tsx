...
export default function IoDCategoryPage() {
  ...
  const category = params.category as IoDCategory;

  const [items, setItems] = useState<IoDItem[]>([]);
  ...
  const fetchItems = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/iod?category=${category}`);
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 text-text-muted text-sm mb-1">
            ...
            <span className="text-text">{IOD_CATEGORY_LABELS[category]}</span>
          </div>
          <h1 className="text-3xl font-bold text-text">{IOD_CATEGORY_LABELS[category]}</h1>
          {!loading && (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-xl font-semibold text-text mb-2">No {IOD_CATEGORY_LABELS[category]} Yet</h2>
