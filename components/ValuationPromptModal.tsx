...
export default function ValuationPromptModal({
  item,
  onClose,
  onValuationComplete,
}: ValuationPromptModalProps) {
        ...
        {step === "loading" && (
          <div className="p-10 text-center">
            ...
            <p className="text-text-muted text-sm">
              Searching eBay & Reverb for comparable sales of{" "}
              <span className="text-text">{itemName}</span>…
            </p>
            ...
            <>
              ...
              <div className="overflow-y-auto flex-1 p-6 space-y-5">
                ...
                <div className="bg-surface-2 rounded-xl p-5 border border-border text-center">
                  <p className="text-sm text-text-muted mb-1">Estimated Resale Value</p>
                  <p className="text-4xl font-bold text-text">{formatPrice(result.suggested_price)}</p>
                  <p className="text-sm text-text-muted mt-1">
                    Range: {formatPrice(result.price_low)} – {formatPrice(result.price_high)}
                  </p>
                </div>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {sales.length > SALES_PER_PAGE && (
                        <span className="text-xs text-text-dim">
                          {salesPage * SALES_PER_PAGE + 1}–{Math.min((salesPage + 1) * SALES_PER_PAGE, sales.length)} of {sales.length}
                        </span>
                      )}
                    </div>
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              {sale.listing_type === "for_sale" ? (
                                <span className="text-xs font-medium px-1.5 py-0.5 rounded-md bg-orange-500/10 text-orange-400 border border-orange-500/20">For Sale</span>
                              ) : sale.listing_type === "sold" ? (
                                <span className="text-xs font-medium px-1.5 py-0.5 rounded-md bg-green-500/10 text-green-400 border border-green-500/20">Sold</span>
                              ) : null}
                              {sale.condition && (
                                <span className="text-xs text-text-dim">{sale.condition}</span>
                              )}
                            </div>
                            {sale.url ? (
                            ...
                            {sale.date && (
                              <p className="text-xs text-text-dim mt-0.5">{sale.date}</p>
                            )}
                          </div>
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between mt-3">
                        <button
                          ...
                          className="flex items-center gap-1 text-xs text-text-dim hover:text-text disabled:opacity-30 disabled:cursor-not-allowed transition-colors px-2 py-1 rounded-lg hover:bg-surface-3"
                        >
