...
interface AddWatchModalProps {
  ...
  onClose: () => void;
  onItemAdded: (item: WatchItem, offerValuation?: boolean) => void;
}
...
export default function AddWatchModal({
  defaultCategory,
  onClose,
  onItemAdded,
}: AddWatchModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newItem: WatchItem = await res.json();
  ...
  return (
    <ModalShell
      title="Add New Watch"
      subtitle={WATCH_CATEGORY_LABELS[form.category]}
      onClose={onClose}
    >
